...
/** ALL-CAPS marketing slogan that usually starts Lazada Description. */
...
/**
 * Tokenize mashed highlight blob like:
 * "STYLE COLOR: 415445-102 Men's Footwear Training White/Metallic Silver Nike Men's Air Monarch IV Shoes"
 */
function tokenizeHighlightBlob(before: string, productTitle?: string): string[] {
  ...
  // Category words before a colorway (Footwear Training …)
  ...
  // Leftover short tokens only — skip long title residue
...
/**
 * Lazada OG / PDP text often looks like:
 *   Highlights
 *   - STYLE COLOR: ...
 *   Description
 *   Marketing paragraphs...
 *
 * Or mashed on one line without headers:
 *   STYLE COLOR: 415445-102 Men's Footwear … ShoesTOTAL SUPPORT FOR MOVING IN COMFORT. …
 */
export function splitProductCopy(
  ...
  if (hasHighlights || hasDescription) {
    ...
    // If highlights were mashed onto few lines, tokenize further
    const highlights =
  ...
  // No headers — mashed OG string (common on Lazada Thailand)
  if (/STYLE\s*COLOR\s*:/i.test(text) || findSloganStart(text) > 8) {
    ...
    // STYLE COLOR present but no slogan — still peel highlights from the front
    if (/^STYLE\s*COLOR\s*:/i.test(text)) {
      const highlights = tokenizeHighlightBlob(text, productTitle);
      // Description = remainder after removing highlight tokens from the start roughly via slogan/title
  ...
  // Multi-line without headers: short lines = highlights, long block = description
